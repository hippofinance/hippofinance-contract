...
contract DarkHippo is ERC20("DarkHippoV2", "dHIPPOv2"), Ownable {
    ...
    constructor(
        ...
        _mint(_msgSender(), 100 * 1000000000000000000);
    ...
    function stakeLP(uint256 amount) public updateStakingRewardLP(_msgSender()) {
        ...
        emit StakedLP(_msgSender(), amount, totalStakedAmountLP);
    }

    // Unstake $HIPPO/ETH
    function unstakeLP(uint256 amount) public updateStakingRewardLP(_msgSender()) {
        require(isStart, "not started");
        require(amount <= staked[_msgSender()].stakedHippoLP, ":unstakeLP: Fund ERROR");
